/** Emission Lighting -- GLSL Fragment Shader */
#version 330
...
vec4 calculateLightColor (
    in Light_Info lights[kMaxLights],
    in ivec4 offsets,
    in vec3 normal,
    in vec3 position,
    in vec3 eyePosition,
    in Material mat)
{
    ...
    for (kMax = offsets[1]; k < kMax; ++k) {
        ...
        lightColor +=
          calculateDirectionalLight(lights[k], normal, position, eyePosition, mat);
#endif
    }
...
#if ENABLE_POINT
        lightColor +=
          calculatePointLight(lights[k], normal, position, eyePosition, mat);
#endif
    }
...
// Sort lights into ambient, directional, spot, point and store offsets
// in a vec4 such that:
//    0 .. < [0] = ambient
//  [0] .. < [1] = directional
//  [1] .. < [2] = spot
//  [2] .. < [3] = point
// Don't branch on light type, all lights have the same info, just iterate
// array slices.
layout (std140) uniform LightingBlock {
    ivec4 offsets;
    Light_Info data[kMaxLights];
} lights;
...
void main ()
{
    vec4 baseColor = texture(sampler, texCoord0);
